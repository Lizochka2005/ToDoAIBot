@dp.message(Command("my_tasks_for_date"), Start.question)
...
@dp.message(GetTaskListForDate.waiting_for_date)
async def show_tasks_for_date(message: Message, state: FSMContext):
  date = message.text
  try:
    a = date.split('-')
    if len(a)!=3 or len(a[0])!=4 or len(a[1])!=2 or len(a[2])!=2 or a[0][:1]==0:
  ...
  async with aiosqlite.connect('users.db') as db:
      async with db.execute("SELECT task, status, date, time FROM tasks WHERE user_id=? and date=?", (user_id, date,)) as cursor:
